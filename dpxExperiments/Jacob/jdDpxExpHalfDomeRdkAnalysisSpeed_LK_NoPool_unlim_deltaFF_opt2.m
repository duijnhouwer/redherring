...
subplot(1,3,3,'align');
curves=dpxdSplit(curves,'speeds');
difcurvesyaw=abs((curves{1,2}.yaw)-(curves{1,1}.yaw));
difcurvesyawSem=zeros(size(difcurvesyaw));  %abs((curves{1,2}.yawSem)-(curves{1,1}.yawSem))
for i=1:numel(curves)
    % select the color for this line from the colmap
...
%     freezeflipUsed=[freezeflipUsed -1];     % add -1 to analyze the pooled data also
    freezeflipUsed=[2 -1];                      %ONLY LOOK AT freezeflip 2
