function jdDpxExpHalfDomeRdkAnalysisSpeedRobustness
    ...
    for fi=1:numel(flds)
        ...
        for i=1:numel(uSpeed)
            v(i)=uSpeed(i); %#ok<AGROW>
            y(i)=mean(K.yaw(K.speeds==uSpeed(i))); %#ok<AGROW>
            ySem(i)=std(K.yaw(K.speeds==uSpeed(i)))./sqrt(sum(K.speeds==uSpeed(i))); %#ok<AGROW>
        end
        lineHandles(fi) = errorbar(v,y,ySem,lstl,'Color',col,'LineWidth',2,'Marker',mrk,'MarkerSize',10,'MarkerEdgeColor','none','MarkerFaceColor',col); %#ok<AGROW>
    end
